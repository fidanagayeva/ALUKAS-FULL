...
const AutumnCollection = () => {
    const [autumnItems, setAutumnItems] = useState([]);
    const [wishlist, setWishlist] = useState([]);
    const [loading, setLoading] = useState(true); 
    const [error, setError] = useState(null); 

    useEffect(() => {
        const fetchAutumnItems = async () => {
            try {
                const response = await fetch('http://localhost:3001/api/autumn');
                if (!response.ok) {
                    throw new Error('Network response was not ok');
                }
                const result = await response.json();
                setAutumnItems(result);
            } catch (error) {
                console.error('Error fetching autumn items:', error);
                setError('Failed to load autumn items. Please try again later.');
            } finally {
                setLoading(false);
            }
        };
    ...
    return (
        <div className="autumn-collection-container px-8 mt-10 mb-[7rem]">
            <h2 className="text-4xl font-sans text-center mb-8">Autumn Collection</h2>
            <p className="text-[1.2rem] font-sans text-gray-600 text-center mb-8">Collect your loves with our newest arrivals.</p>

            {Array.isArray(autumnItems) && autumnItems.length > 0 ? (
                <Swiper
                    ...
                    className="autumn-swiper"
                >
                    {autumnItems.map(item => (
